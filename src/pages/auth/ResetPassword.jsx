...
export default function ResetPassword() {
  const { pendingReset, resetPassword, loading, error, clearError } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const ok = await resetPassword(password, confirm);
    if (ok) setDone(true);
  };

  // Guard: if no reset token yet, redirect to step 1
  if (!pendingReset?.resetToken && !done) {
    return (
      <AuthLayout>
        <div className="text-center py-6">
          ...
          <p className="text-gray-500 text-sm mt-2 leading-relaxed">
            Your password reset session is invalid or has expired. Please start over.
          </p>
          <Link
            to="/auth/forgot-password"
            className="inline-flex items-center gap-2 mt-6 px-5 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-gray-800 transition-colors"
          >
            Start over
          </Link>
        </div>
    ...
    <AuthLayout>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Resetting for{' '}
          <span className="font-semibold text-gray-700">{pendingReset?.sentTo ?? 'your account'}</span>
        </p>
      </div>
      ...
      <p className="text-center text-xs text-gray-400 mt-2">Step 3 of 3</p>
    </AuthLayout>
  );
}
